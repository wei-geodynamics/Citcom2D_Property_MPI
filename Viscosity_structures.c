...

{
    ...
    ViscCentre = (double *)malloc((E->mesh.nno + 10) * sizeof(double));
...

{
    ...
    double x_ocean_front[10000], x_ocean_front_con[10000];
    void element_markers();
    void get_C_from_markers_multi();
            ...
            for (jj = 1; jj <= vpts; jj++)
            {
                ...
                if (E->viscosity.visc_platebond)
                {
                    if (E->control.imposevelo)
                    {
                        ...
                        velobegin = E->VB[1][E->mesh.noz * 7];
                        newnum = 8;
                        while (newnum <= E->mesh.nox - 1 && E->VB[1][E->mesh.noz * newnum] > velobegin - 910.0)
                        {
                            newnum++;
                        }
                        markvnum = newnum;
                        if (E->X[2][E->ien[i].node[jj]] >= E->viscosity.z_weakzone_platebond && E->X[1][E->ien[i].node[jj]] - E->X[1][markvnum * E->mesh.noz] >= 0.0 - E->viscosity.width_weakzone_platebond && E->X[1][E->ien[i].node[jj]] - E->X[1][markvnum * E->mesh.noz] <= E->viscosity.width_weakzone_platebond)
                        {
                            visc_weakzone = E->control.visc_weakzone;
                        }
                    ...
                    if (E->control.trechmigrate)
                    {
                        ...
                        if (E->control.initialTOption == 1)
                        {
                            ...
                            tempdist_left = (t1 - loc_mid) * tan(E->control.dip_margin_left * 3.14159265 / 180.0) + 1.0 - r1;
                            if (tempdist_left >= (0.0 - E->viscosity.left_weakzone_platebond) *
                                                     tan(E->control.dip_margin_left * 3.14159265 / 180.0) &&
                                tempdist <= E->viscosity.right_weakzone_platebond *
                                                tan(E->control.dip_margin * 3.14159265 / 180.0) &&
                ...
                if (E->control.phasevisc)
                {
                    ...
                    {
                        newelz = ((i - 1) % E->mesh.elz) + 1;
                        nz = (E->ien[i].node[jj] % E->mesh.noz) + 1;
...
*/
    ...
    case 10:
        ...
        element_markers(E, E->XMC, E->CElement);
        ...
        for (i = 1; i <= nel; i++)
        {
            ...
            for (jj = 1; jj <= vpts; jj++)
            {
                ...
                if (E->X[2][E->ien[i].node[jj]] >= E->viscosity.z_weakzone_platebond)
                {
                    ...
                    temp_visc_in = cos(PI / 2.0 * tanh(temp_dist / E->viscosity.left_weakzone_platebond * E->viscosity.left_weakzone_platebond_ratio));
        ...
        for (i = 1; i <= nel; i++)
        {
            ...
            for (jj = 1; jj <= vpts; jj++)
            {
                ...
                if (r1 >= E->viscosity.z_weakzone_platebond)
                {
                    ...
                    temp_visc_log = log(E->viscosity.visc_reduce_platebond);
                    temp_visc_in = cos(PI / 2.0 * tanh(temp_dist / E->viscosity.left_weakzone_platebond * E->viscosity.left_weakzone_platebond_ratio / temp_ratio));
                    visc_weakzone = exp(temp_visc_in * temp_visc_log);
                }
                if (E->control.visc_leftcor)
                {
        ...
        for (jj = 1; jj <= vpts; jj++)
        {
            for (m = 0; m < E->control.phasefile_C_flavor; m++)
            {
                ...
                else
                {
                    ...
                    scale = pow(eedot[e] / E->viscosity.sdepv_trns[m], exponent1);
                    temp3 = EEta[(e - 1) * vpts + jj] / (one + scale * pow(EEta[(e - 1) * vpts + jj], exponent1));
                }
