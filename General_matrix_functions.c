...
double **fmatrix(nrl,nrh,ncl,nch)
     int nrl,nrh,ncl,nch;
{
  int i,nrow = nrh-nrl+1,ncol=nch-ncl+1;
  double **m;
 ...
 */   
            } while ((residual > acc || count<E->control.min_vel_iterations) && count<E->control.max_vel_iterations);
	}
	...
	fprintf(E->fp,"%s residual (%d)(%d) = %.3e from %.3e to %.3e in %5.2f secs \n",
	...
	printf("Total time for %d loops = %lf \n",count,CPU_time0()-initial_time);
	for(i=E->mesh.levmax;i>=E->mesh.levmin;i--) {
	    printf("Level %d, total time = %lf\n",i,cost_per_level[i]);
	    printf("     total cycles = %d (%lf)\n",total_cycles[i],
		   cost_per_level[i]/(1.0e-5+(double)total_cycles[i]));
      }
	printf("projection time = %lf\n",E->monitor.cpu_time_on_mg_maps);
	printf("Del sq u solved to accuracy of  %lf \n",residual);
    }
...
{ double det;
 ...
 double area_of_4node(x1,y1,x2,y2,x3,y3,x4,y4)
