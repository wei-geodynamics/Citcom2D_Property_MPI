...
static double cost_per_level[MAX_LEVELS]; /* this will accumulate data over the run */
...
double **fmatrix(nrl, nrh, ncl, nch) int nrl, nrh, ncl, nch;
{
  int i, nrow = nrh - nrl + 1, ncol = nch - ncl + 1;
  double **m;
...
void dfree_matrix(m, nrl, nrh, ncl, nch) double **m;
int nrl, nrh, ncl, nch;
{
  int i;
  for (i = nrh; i >= nrl; i--)
...
void ffree_matrix(m, nrl, nrh, ncl, nch) double **m;
int nrl, nrh, ncl, nch;
{
  int i;
  for (i = nrh; i >= nrl; i--)
...
double *fvector(nl, nh) int nl, nh;
{
  double *v;
  v = (double *)malloc((unsigned)(nh - nl + 1) * sizeof(double));
  return (v - nl);
...
void dfree_vector(v, nl, nh) double *v;
int nl, nh;
{
  free((char *)(v + nl));
}

void ffree_vector(v, nl, nh) double *v;
int nl, nh;
{
  free((char *)(v + nl));
}

int *sivector(nl, nh) int nl, nh;
{
  int *v;
...
void sifree_vector(v, nl, nh) int *v;
int nl, nh;
{
  free((char *)(v + nl));
}

double pdot(E, A, B, lev) struct All_variables *E;
double *A, *B;
int lev;
...
double pselfdot(E, A) struct All_variables *E;
double *A;
...
double fvdot(E, A, B, level) struct All_variables *E;
double *A, *B;
int level;
{
  double prod, mprod[1];
...
double vdot(E, A, B, level) struct All_variables *E;
double *A, *B;
int level;
{
  double prod, mprod[1];
...
double vselfdot(E, A, level) struct All_variables *E;
double *A;
int level;
{
  double prod;
...
double vfselfdot(E, A, level) struct All_variables *E;
double *A;
int level;
...
double fdot(A, B, n1, n2) double *A, *B;
int n1, n2;
...
double fselfdot(A, n1, n2) double *A;
int n1, n2;
...
double dot(E, A, B) struct All_variables *E;
double *A, *B;
...
double selfdot(E, A) struct All_variables *E;
double *A;
...
void dvcopy(A, B, a, b) double *A, *B;
int a, b;
...
void vprod(R, A, B, a, b) double *R, *A, *B;
int a, b;
...
double fnmax(E, A, a, b) struct All_variables *E;
double *A;
int a, b;
...
int solve_del2_u(E, d0, F, acc, high_lev) struct All_variables *E;
double *d0;
double *F;
double acc;
int high_lev;
{
  ...
  if (!(E->control.NMULTIGRID || E->control.EMULTIGRID))
  {
    ...
    time = CPU_time0();
    residual = conj_grad(E, D1, r, Au, acc * E->control.relaccCG,
                         &cycles, high_lev);
    for (i = 0; i < neq; i++)
    {
      d0[i] += D1[i];
 ...
 */
  ...
  { /* multigrid  */
    ...
    do
    {
      residual = multi_grid(E, D1, r, Au, acc * E->control.relaccMG,
                            high_lev);
      ...
      if (E->monitor.solution_cycles % 5 == 0 && E->parallel.me == 0)
      {
        fprintf(E->fp, "Mresi = %.6e for iter %d acc %.6e\n",
                residual, count, acc);
      }
    } while (residual > acc * E->control.relaccMG);
  }

  if ((count > 0) &&
          (residual > r0 * 2.0) ||
      (fabs(residual - prior_residual) < acc * 0.1 &&
       (residual > acc * 10.0)))
    convergent = 0;
  else
  {
  ...
  {
    fprintf(E->fp,
            "%s residual (%03d)(%03d)=%.3e from %.3e to %.3e in %5.2f secs\n",
            (convergent ? " * " : "!!!"), count, cycles, residual, r0, acc,
            CPU_time0() - initial_time);
    fflush(E->fp);
  }
  ...
  if (E->control.verbose)
  {
    ...
    for (i = E->mesh.levmax; i >= E->mesh.levmin; i--)
    {
      ...
      printf("     total cycles = %d (%g)\n", total_cycles[i],
             cost_per_level[i] / (1.0e-5 + (double)total_cycles[i]));
    }
...
double multi_grid(E, d1, F, Au, acc, hl) struct All_variables *E;
double *d1;
double *F;
double *Au;
double acc;
int hl; /* higher level of two */
{
  ...
  cycles = E->control.v_steps_low;
  resi = conj_grad(E, vel[levmin], fl[levmin], AU[levmin],
                   acc * E->control.relaccCG, &cycles, levmin);
  ...
  for (lev = levmin + 1; lev <= levmax; lev++)
  {
    ...
    for (Vn = 1; Vn <= Vnmax; Vn++)
    {
      ...
      /*    Bottom of the V    */
      cycles = E->control.v_steps_low;
      /*(void)conj_grad(E, vel[levmin], rhs[levmin], AU[levmin], acc * E->control.relaccCG, &cycles, levmin); */
...
double conj_grad(E, d0, F, Au, acc, cycles, level) struct All_variables *E;
double *d0;
double *F;
double *Au;
double acc;
...
int level;
{
  ...
  fprintf(E->fp, "CG: count=%d, residual=%g, acc=%g\n",
          count, residual, acc);
...
void jacobi(E, d0, F, Ad, acc, cycles, level, guess) struct All_variables *E;
double *d0;
double *F, *Ad;
double acc;
int *cycles;
...
void element_gauss_seidel(E, d0, F, Ad, acc, cycles, level, guess) struct All_variables *E;
double *d0;
double *F, *Ad;
double acc;
int *cycles;
...
void gauss_seidel(E, d0, F, Ad, acc, cycles, level, guess) struct All_variables *E;
double *d0;
double *F, *Ad;
double acc;
int *cycles;
...
void gauss_seidel1(E, d0, F, Ad, acc, cycles, level, guess) struct All_variables *E;
double *d0;
double *F, *Ad;
double acc;
int *cycles;
...
void print_elt_k(E, a) struct All_variables *E;
double a[24 * 24];
...
double cofactor(A, i, j, n) double A[4][4];
int i, j, n;
...
double determinant(A, n) double A[4][4];
int n;
...
double gen_determinant(A, n) double **A;
int n;
...
double area_of_4node(x1, y1, x2, y2, x3, y3, x4, y4) double x1, y1, x2, y2, x3, y3, x4, y4;
...
/*
double Tmax(E, T) struct All_variables *E;
double *T;
{
  double temp;
  int i;
...
double Tmin(E, T) struct All_variables *E;
double *T;
{
  double temp;
  int i;
...
double vnorm_nonnewt(E, dU, U, lev) struct All_variables *E;
double *dU, *U;
int lev;
{
...
/* ===============================================
   ...
   =============================================== */
/*
void remove_horiz_ave(E, X, H, store_or_not) struct All_variables *E;
double *X, *H;
int store_or_not;
     ...
     double *X, *H;
{
  ...
  sizeofH = (2*E->mesh.noz+2)*sizeof(double);
  ...
  */
/*
double return_bulk_value(E,Z,average)
     struct All_variables *E;
     double *Z;
     int average;
...
*/
/*double kineticE(E, A, lev) struct All_variables *E;
double *A;
int lev;
