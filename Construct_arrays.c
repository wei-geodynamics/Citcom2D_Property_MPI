...
void construct_ien(E) struct All_variables *E;

{
    ...
    for (r = 1; r <= rmax; r++)
      for (q = 1; q <= qmax; q++)
        for (p = 1; p <= pmax; p++)
        {
          ...
          for (rr = 1; rr <= ends; rr++)
            E->IEN[lev][element].node[rr] = start + offset[rr].vector[1] + offset[rr].vector[0] * pnmax + offset[rr].vector[2] * pnmax * qnmax;
        }
  ...
  for (element = 1; element <= E->lmesh.NEL[E->mesh.levmax]; element++)
    if ((element - 1) % E->lmesh.elz == 0)
    {
      e++;
      E->sien[e].node[1] = (E->ien[element].node[1] - 1) / E->lmesh.noz + 1;
      E->sien[e].node[2] = (E->ien[element].node[4] - 1) / E->lmesh.noz + 1;
      if (dims == 3)
      {
        E->sien[e].node[3] = (E->ien[element].node[8] - 1) / E->lmesh.noz + 1;
        E->sien[e].node[4] = (E->ien[element].node[5] - 1) / E->lmesh.noz + 1;
      }
      E->surf_element[e] = element;
    }
  E->lmesh.snel = e;
  for (i = 1; i <= E->lmesh.nsf; i++)
    E->surf_node[i] = (i - 1) * E->lmesh.noz + 1;
...
void construct_id(E) struct All_variables *E;
{
  ...
  for (lev = E->mesh.levmax; lev >= E->mesh.levmin; lev--)
  {
    ...
    k1 = 1;
    k2 = noy; /* default for 2D */
    if (E->mesh.nsd == 3)
    {
      k1 = 1;
      k2 = noy - 1;
      if (E->parallel.me_loc[3] == E->parallel.nprocy - 1)
        k2 = noy;
    }
...
void construct_lm(E) struct All_variables *E;
{
  int i, a, e;
  int lev, eqn_no;
  int nel, nel2;
...
/* =====================================================
   ...
   Taking into account of the symmetry of stiffness matrix.
   Only half of the matrix is stored.
   ===================================================== */

void construct_node_maps(E) struct All_variables *E;
{
  double initial_time, CPU_time();
...
void construct_node_ks(E) struct All_variables *E;
{
  int level, i, j, k, e;
  ...
  for (lev = E->mesh.levmax; lev >= E->mesh.levmin; lev--)
  {
    ...
    if (E->mesh.nsd == 3)
    {
      if (E->parallel.me_loc[3] == 0)
        for (i = 1; i <= noz; i++) /* Front-side edge */
          for (j = 1; j <= nox; j++)
          {
            node = i + (j - 1) * noz;
      ...
      if (E->parallel.me_loc[3] == E->parallel.nprocy - 1)
        for (i = 1; i <= noz; i++) /* Back-side edge */
          for (j = 1; j <= nox; j++)
          {
    ...
    if (E->parallel.me_loc[1] == 0) /* Left edge */
      for (i = 1; i <= noz; i++)
        for (j = 1; j <= noy; j++)
        {
          node = i + (j - 1) * noz * nox;
...
void construct_sub_element(E) struct All_variables *E;
    ...
    for (i = 1; i <= elx; i++)
      for (j = 1; j <= elz; j++)
        for (k = 1; k <= ely; k++)
        {
          ...
          {
            E->EL[lev][elt].sub[l] = eltu + offset[l].vector[1] + offset[l].vector[0] * elzu + offset[l].vector[2] * elzu * elxu;
          }
        }
  }
...
void construct_elt_ks(E) struct All_variables *E;
{
  int e, el, lev, j, k, ii;
  void get_elt_k();
...
void construct_elt_gs(E) struct All_variables *E;
{
  int el, lev, a;
  void get_elt_g();
...
void construct_mat_group(E) struct All_variables *E;
{
  int layers();
  double t_b;
...
void construct_stiffness_B_matrix(E) struct All_variables *E;
{
  void vcopy();
  void build_diagonal_of_K();
  ...
  int i;
//        fprintf(stderr, "before project%d \n", E->parallel.me);
...
//        fprintf(stderr, "project 1 %d \n", E->parallel.me);
//        fprintf(stderr, "project 12 %d %d %d\n", E->parallel.me, E->control.KERNEL, E->monitor.solution_cycles);
   
  if (been_here0 == 0 || (E->viscosity.update_allowed))
  {
//        fprintf(stderr, "project 01 %d \n", E->parallel.me);

    /* do the following for the 1st time or update_allowed is true */
//        fprintf(stderr, "project 2 %d \n", E->parallel.me);

    if (E->control.NMULTIGRID)
      project_viscosity(E);
//        fprintf(stderr, "finish viscosity stiff%d \n", E->parallel.me);

    construct_elt_gs(E);
//        fprintf(stderr, "finish elt_gs%d \n", E->parallel.me);
...
//        fprintf(stderr, "finish ks%d \n", E->parallel.me);

    build_diagonal_of_Ahat(E);
//        fprintf(stderr, "finish Ahat%d \n", E->parallel.me);

    if (E->control.NMULTIGRID || E->control.NASSEMBLE)
      rebuild_BI_on_boundary(E);
//        fprintf(stderr, "finish BIBC%d \n", E->parallel.me);

    been_here0 = 1;
  }
//        fprintf(stderr, "after Bmatrix%d \n", E->parallel.me);

  return;
}

void rebuild_BI_on_boundary(E) struct All_variables *E;
{
  int m, level, i, j;
  int eqn1, eqn2, eqn3;
    ...
    for (i = 1; i <= E->lmesh.NNO[level]; i++)
      if (E->NODE[level][i] & LIDN)
      {
        ...
        else
        {
          ...
          for (j = 0; j < max_eqn; j++)
            E->temp[C[j]] += fabs(B1[j]) + fabs(B2[j]);
        }
      }
