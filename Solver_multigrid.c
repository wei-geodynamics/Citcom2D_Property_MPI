...
void set_mg_defaults(E) struct All_variables *E;
{
...
void mg_allocate_vars(E) struct All_variables *E;
{
    return;
}

void project_vector(E, start_lev, AU, AD, ic) struct All_variables *E;
int start_lev, ic;
...
/* ==================================================== */
void project_scalar_e(E, start_lev, AU, AD) struct All_variables *E;
int start_lev;
double *AU, *AD; /* data on upper/lower mesh  */
{
    int i, j, m;
...
/* ==================================================== */
void project_scalar_e_geom(E, start_lev, AU, AD)
    /* As project_scalar_e, but using a geometric average for
      * viscosity averaging: eta_D = (PRODUCT eta_U)^weight */
    struct All_variables *E;
int start_lev;
double *AU, *AD; /* data on upper/lower mesh  */
{
...
void project_viscosity(E) struct All_variables *E;
...
//        fprintf(stderr, "before malloc %d \n", E->parallel.me);
    ...
    viscU = (double *)malloc((1 + E->lmesh.NNO[E->mesh.levmax]) * sizeof(double));
    viscD = (double *)malloc((1 + vpts * E->lmesh.NNO[E->mesh.levmax - 1]) * sizeof(double));
    ...
    for (lv = E->mesh.levmax; lv > E->mesh.levmin; lv--)
    {
        ...
        else if (E->viscosity.smooth_cycles == 4)
        {
            visc_from_gint_to_elel(E, E->EVI[lv], viscU, lv);
            project_scalar_e_geom(E, lv, viscU, viscD);
            visc_from_ele_to_gint(E, viscD, E->EVI[sl_minus], sl_minus);
        }
        else if (E->viscosity.smooth_cycles == 0)
        {
//        fprintf(stderr, "before from g to nodes %d \n", E->parallel.me);

            visc_from_gint_to_nodes(E, E->EVI[lv], viscU, lv);

//        fprintf(stderr, "before inject %d \n", E->parallel.me);

            inject_scalar(E, lv, viscU, viscD);
//        fprintf(stderr, "before nodes to gint %d \n", E->parallel.me);
...
void inject_scalar(E, start_lev, AU, AD) struct All_variables *E;
int start_lev;
double *AU, *AD; /* data on upper/lower mesh  */
