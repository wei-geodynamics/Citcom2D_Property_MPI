			...
			if ((j == 1 && E->parallel.me_loc[2] != 0) || (j == 2 && E->parallel.me_loc[2] != E->parallel.nprocz - 1))
			{
				for (k = 1; k <= noy; k++)
					for (i = 1; i <= nox; i++)
					{
						...
						llnode = i + (k - 1) * nox;
						E->parallel.NODE[lev][llnode].bound[2][j] = node;
						E->NODE[lev][node] = E->NODE[lev][node] | OFFSIDE;
						E->NODE[lev][node] = E->NODE[lev][node] | LIDN;
						node1 = node + (((j == 1) ? 1 : -1));
						E->NODE[lev][node1] = E->NODE[lev][node1] | LIDN;
...
void exchange_node_f20(struct All_variables *E, double *U, int lev)
{
	//int target_proc, kk, e, node, i, ii, j, k, bound, type, idb, msginfo[8];
	int target_proc, kk, i, j, k, idb;
	static double *S[27], *R[27];
	...
	if (been_here == 0)
	{
		...
		for (i = 1; i <= E->mesh.nsd; i++)
			...
			{
				if (E->parallel.NUM_PASS[levmax].bound[i][k])
					sizeofk = max(sizeofk, (1 + E->parallel.NUM_NODE[levmax].pass[i][k]) * sizeof(double));
			}
	...
	for (i = 1; i <= E->mesh.nsd; i++)
	{
		...
		for (k = 1; k <= 2; k++)
			if (E->parallel.NUM_PASS[levmax].bound[i][k])
			{
				...
				{
					idb++;
					MPI_Isend(S[k], E->parallel.NUM_NODE[lev].pass[i][k], MPI_DOUBLE, target_proc, 1, MPI_COMM_WORLD, &request[idb - 1]);
				}
			} /* for k */
		...
		for (k = 1; k <= 2; k++)
			if (E->parallel.NUM_PASS[levmax].bound[i][k])
			{
				...
				{
					idb++;
					MPI_Irecv(R[k], E->parallel.NUM_NODE[lev].pass[i][k], MPI_DOUBLE, target_proc, 1, MPI_COMM_WORLD, &request[idb - 1]);
				}
			} /* for k */
