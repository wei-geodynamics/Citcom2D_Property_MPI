...
#include <mpi.h>
void twiddle_thumbs(yawn, scratch_groin) struct All_variables *yawn;
int scratch_groin;
...
void get_global_shape_fn(E, el, pressure, level) struct All_variables *E;
int el;
int pressure, level;
{
  int i, j, k, d, e;
        ...
        for (d = 1; d <= dims; d++)
          for (e = 1; e <= dims; e++)
          {
            ...
            /*if (level == E->mesh.levmax)
              fprintf(stderr, "dxda %d %d %.4e %.4e\n", d, E->IEN[level][el].node[i], E->XX[level][e][E->IEN[level][el].node[i]], E->Nx.vpt[GNVXINDEX(d - 1, i, k)]);*/
          }

      jacobian = determinant(dxda, E->mesh.nsd);
      E->GDA[level][el].vpt[k] = jacobian;
      //fprintf(stderr, "J %d %.4e \n", el, jacobian);
      for (d = 1; d <= dims; d++)
        for (e = 1; e <= dims; e++)
...
void get_global_shape_fn1(E, el, GN, GNx, dOmega, pressure, level) struct All_variables *E;
int el;
struct Shape_function *GN;
struct Shape_function_dx *GNx;
struct Shape_function_dA *dOmega;
...
void get_global_1d_shape_fn(E, el, GM, dGammax, nodal) struct All_variables *E;
int el, nodal;
...
struct Shape_function1_dA *dGammax;
{
  ...
  for (locn = 0; locn <= 1; locn++) /* top/bottom, front/back, left/right */
    for (dirn = 1; dirn <= E->mesh.nsd; dirn++)
    {
      ...
      collapsed_dirn[0] = 1;
      collapsed_dirn[1] = 3; /* includes 3d  */
      if (collapsed_dirn[0] == (dirn))
        collapsed_dirn[0]++;
      else if (collapsed_dirn[1] == (dirn))
        collapsed_dirn[1]--;
...
void get_neighbour_nodes(node, dirn, locn) int node[5];
...
void mass_matrix(E) struct All_variables *E;

{
  ...
  double temp[9], area, centre[4];
  double dx1, dx2, dx3, xlowmean, normlow, normhigh, xhighmean;
  struct Shape_function GN;
  struct Shape_function_dA dOmega;
  struct Shape_function_dx GNx;
  ...
  for (lv = E->mesh.levmax; lv >= E->mesh.levmin; lv--)
  {
    ...
    {
      get_global_shape_fn(E, el, 0, lv);
      get_global_shape_fn(E, el, 2, lv);
    }
    ...
    for (e = 1; e <= E->lmesh.NEL[lv]; e++)
    {
      ...
      /*      get_global_shape_fn(E,e,&GN,&GNx,&dOmega,0,lv);
*/
      ...
      if (3 == E->mesh.nsd)
      {
        dx1 = 0.25 * (E->XX[lv][1][n[3]] + E->XX[lv][1][n[4]] + E->XX[lv][1][n[7]] + E->XX[lv][1][n[8]] - E->XX[lv][1][n[1]] - E->XX[lv][1][n[2]] - E->XX[lv][1][n[5]] - E->XX[lv][1][n[6]]);
        E->ECO[lv][e].size[1] = sqrt(dx1 * dx1);

        dx2 = 0.25 * (E->XX[lv][2][n[2]] + E->XX[lv][2][n[3]] + E->XX[lv][2][n[6]] + E->XX[lv][2][n[7]] - E->XX[lv][2][n[1]] - E->XX[lv][2][n[4]] - E->XX[lv][2][n[5]] - E->XX[lv][2][n[8]]);
      ...
      {
        dx1 = 0.5 * (E->XX[lv][1][n[3]] + E->XX[lv][1][n[4]] - E->XX[lv][1][n[1]] - E->XX[lv][1][n[2]]);
        dx2 = 0.5 * (E->XX[lv][2][n[3]] + E->XX[lv][2][n[4]] - E->XX[lv][2][n[1]] - E->XX[lv][2][n[2]]);
        E->ECO[lv][e].size[1] = sqrt(dx1 * dx1 + dx2 * dx2);

        dx1 = 0.5 * (E->XX[lv][1][n[2]] + E->XX[lv][1][n[3]] - E->XX[lv][1][n[1]] - E->XX[lv][1][n[4]]);
        dx2 = 0.5 * (E->XX[lv][2][n[2]] + E->XX[lv][2][n[3]] - E->XX[lv][2][n[1]] - E->XX[lv][2][n[4]]);
  ...
  fprintf(stderr, "Volume %.4e %.4e \n", E->lmesh.volume, E->mesh.volume);
